...
export default async function Home() {
  console.log('[Home] Iniciando renderização segura...');
  // const session = await getServerSession(authOptions).catch(e => {
  //   console.error('[Home] Erro ao buscar sessão:', e);
  //   return null;
  // });
  ...
  try {
    ...
    try {
      if (etapaAtiva) {
        roundAtivo = await prisma.round.findFirst({ 
          where: { etapaId: etapaAtiva.id },
          orderBy: { id: 'desc' } 
        });
        if (roundAtivo) {
      ...
      <div className="landing-body" style={{ fontFamily: config?.fontFamily || 'Inter' }}>
        ...
        {layout.filter(block => block.visible !== false).map((block, bIdx) => {
          ...
          if (elements && elements.length > 0) {
            return (
              <section key={block.id || `section-${bIdx}`} style={{
                height: '500px',
    ...
    console.error('CRITICAL_HOME_RENDER_ERROR:', criticalError);
    return (
      <div style={{ padding: '100px', textAlign: 'center', background: '#000', color: '#fff', minHeight: '100vh' }}>
        <h1 style={{ color: '#ff4444' }}>Ocorreu um erro ao carregar a página inicial.</h1>
        <p>Isso pode ser devido a uma manutenção no banco de dados. Por favor, tente novamente em alguns instantes.</p>
        <div style={{ marginTop: '20px', padding: '20px', background: '#111', borderRadius: '10px', fontSize: '0.8rem', opacity: 0.5 }}>
           DEBUG: {(criticalError as Error).message}
        </div>
      </div>
    );
  }
