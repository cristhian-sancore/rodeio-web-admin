  ...
  try {
    ...
    if (!round) return (
      <div style={{ padding: '100px', textAlign: 'center' }}>
        <h2>Round não encontrado</h2>
        <Link href={`/admin/etapas/${id}`} className="btn-primary" style={{ marginTop: '2rem', display: 'inline-block' }}>Voltar para Etapa</Link>
      </div>
    );

    const hasPreviousRound = round.numero > 1;
      ...
      <div className="fade-in">
        ...
        <div style={{ marginBottom: '2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', margin: '4px 0 0' }}>{round.etapa?.nome || 'Etapa não informada'} • Defina os confrontos entre atletas e animais.</p>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {(reservas || []).map((r: any) => (
                  <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', background: '#050505', padding: '0.5rem', borderRadius: '4px', border: '1px solid #111' }}>
                    <span style={{ fontSize: '0.8rem' }}>#{r.ordem} {r.animal?.nome || 'Animal removido'}</span>
                ...
                <div key={m.id} className="premium-card" style={{ padding: '1rem', position: 'relative', background: '#0a0a0a', border: '1px solid #1a1a1a' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                     <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '0.8rem', fontWeight: '900', color: '#fff' }}>{m.competidor?.nome || 'Não identificado'}</div>
                  ...
                  <div style={{ background: '#000', padding: '0.75rem', borderRadius: '8px', border: '1px solid #111', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                     ...
                     <div>
                        <div style={{ fontSize: '0.85rem', color: 'var(--primary)', fontWeight: '900' }}>{m.animal?.nome || 'Animal removido'}</div>
    ...
    return (
      <div style={{ padding: '100px', textAlign: 'center' }}>
         ...
         <div style={{ marginTop: '2rem' }}>
          <Link href={`/admin/etapas/${id}`} className="btn-primary">Voltar para Etapa</Link>
         </div>
      </div>
    );
  }
