...
import { importCompetidores } from './actions';
...
export default function ExcelActions({ data }: { data: any[] }) {
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, "competidores_rodeio.xlsx");
  };
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const res = await importCompetidores(jsonData);
        if (res.success) {
          setSuccess(`${res.count} competidores importados com sucesso!`);
