...
export default async function ExecucaoPage({ searchParams }: { searchParams: Promise<{ roundId?: string, montariaId?: string, error?: string }> }) {
  ...
  try {
    ...
    console.error("[ExecucaoPage] Erro Crítico:", error);
    return (
      <div style={{ padding: '100px', textAlign: 'center' }}>
         <h2 style={{ color: '#ff4444' }}>Erro na Célula de Lançamento</h2>
         <p>Ocorreu uma falha ao processar os dados do round ou da montaria.</p>
         <Link href="/admin/execucao" className="btn-primary" style={{ marginTop: '2rem', display: 'inline-block' }}>Tentar Novamente (Voltar Seleção)</Link>
      </div>
    );
  }
}
